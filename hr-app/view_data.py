...
with app.app_context():
    ...
    if not requests:
        print("(目前沒有申請資料)")
    for r in requests:
        print(f"ID: {r.id} | 申請人ID: {r.employee_id} | 目的地: {r.destination} | 狀態: {r.status} | 風險: {r.risk_level} | 預算: {r.money}")
    ...
    if not expenses:
        print("(目前沒有報銷明細)")
    for e in expenses:
        print(f"ID: {e.id} | 申請ID: {e.request_id} | 類別: {e.expense_category} | 項目: {e.expense_name} | 地點: {e.expense_location} | 金額: {e.requested_amount}")
    print("="*50 + "\n")
